...
- `hq_line_characteristics_by_voltage.csv` -- the only line-parameter source: Hydro-Quebec's own
  line-characteristics table (transcribed from an HQ planning document; 69/120/161/230/315/735 kV),
  giving exact per-km positive-sequence r/x/b and characteristic impedance by voltage. Used two ways:
  - `apply_hq_line_characteristics.py` writes r/x/b onto 315/345kV and 735/765kV lines (345 treated
    as 315-tier, 765 as 735-tier) -- the two voltage tiers this project's reduced networks actually
    keep as topology.
  - `st_clair.py` reads its characteristic impedance, reactance and susceptance (L and C derived at
    60Hz) to compute each line's thermal-limit envelope (`s_nom`).
- Every other voltage level keeps PyPSA-Earth's generic default type-matching
  (`config.default.yaml`'s `lines.ac_types`, itself sourced from a German textbook, not
  Quebec-specific) -- see [ASSUMPTIONS_AND_LIMITATIONS.md](ASSUMPTIONS_AND_LIMITATIONS.md). This
  doesn't affect the reduced/solved networks (they only keep >=315kV lines), only the unreduced
...
- `st_clair_curve.csv` -- the St Clair curve (a standard planning-level thermal-limit-vs-length
  relationship for overhead transmission lines), generated by `st_clair.py curve` from the HQ table
  above, used to derive line thermal capacity (`s_nom`) from real length and voltage.
